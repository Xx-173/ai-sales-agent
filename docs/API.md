...
| `/api/sales`                       | `src/routes/sales-sop.ts`        | Lengshan 销售 SOP、Outbox 与人工接管 |
...
- `POST /api/sales/lengshan/events`、`/outbox/claim`、`/actions/result` 与
  `/ticks/due` 是 Lengshan 专用的 HMAC 签名服务间接口，不接受浏览器 Cookie
  作为替代认证。
- `GET /api/sales/policy`、`/sop-plan`、`/traces`、`/metrics` 和
  `/handoffs` 需要登录；人工接管的 resolve 与失败动作 retry 仅限管理员。
- 完整签名、入站事件、Outbox 回执与模板 Key 约定见
  [`LENGSHAN_CONNECTOR.md`](LENGSHAN_CONNECTOR.md)。
