...
function resolveDesktopUrls(): { serverUrl: string; rendererUrl: string } {
  const serverValue =
    ...
    process.env.AI_SALES_SERVER_URL ||
    DEFAULT_SERVER_URL;
  const serverUrl = normalizeHttpUrl(serverValue, 'AI Sales Agent server URL');
  const rendererValue =
    getCliValue('--renderer-url') ||
    process.env.AI_SALES_RENDERER_URL ||
    serverUrl;
  const rendererUrl = normalizeHttpUrl(rendererValue, 'AI Sales Agent renderer URL');
...
function showAbout(): Promise<Electron.MessageBoxReturnValue> {
  const options = {
    ...
    detail: `SOP-governed sales Agent workspace\nVersion ${app.getVersion()}`,
  } as const;
...
function errorPageHtml(failedUrl: string): string {
  ...
  return `<!doctype html>
<html><head><meta charset="utf-8"><title>AI Sales Agent is unavailable</title>
...
</head><body><main><h1>AI Sales Agent 后端未连接</h1><p>请先启动 AI Sales Agent 后端，或检查桌面应用配置的服务地址：</p><p><code>${safeUrl}</code></p><button id="retry">重试连接</button><script>document.getElementById('retry').addEventListener('click',()=>window.miniclawDesktop?.retry())</script></main></body></html>`;
}
...
function createMainWindow(): BrowserWindow {
  ...
  const window = new BrowserWindow({
    ...
    icon: path.join(__dirname, '..', 'assets', 'salesclaw-icon.png'),
    webPreferences: {
...
function createApplicationMenu(): void {
  ...
  const template: MenuItemConstructorOptions[] = [
    ...(process.platform === 'darwin' ? [{ role: 'appMenu' as const }] : []),
    {
      ...
      submenu: [
        { label: 'About AI Sales Agent', click: () => void showAbout() },
        { type: 'separator' },
        { role: 'quit' },
      ],
    },
    viewMenu,
    {
      label: 'Help',
      submenu: [
        { label: 'AI Sales Agent on GitHub', click: () => void openExternalUrl('https://github.com/Xx-173/ai-sales-agent') },
      ],
    },
  ];
